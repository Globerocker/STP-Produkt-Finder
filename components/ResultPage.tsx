...
const ResultPage: React.FC<ResultPageProps> = ({ recommendations, answers, onRestart, onCompare }) => {
  ...
  const directDemoUrl = useMemo(() => {
    if (!calendarBaseUrl) return topProduct.demoUrl;

    if (isHubspotUrl) {
      const separator = calendarBaseUrl.includes('?') ? '&' : '?';
      return `${calendarBaseUrl}${separator}${hubspotParams.toString()}`;
    }

    return calendarBaseUrl;
  }, [calendarBaseUrl, isHubspotUrl, hubspotParams, topProduct.demoUrl]);
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="text-center mt-12 animate-fade-in-delay-3">
        ...
        <button
          onClick={() => {
            trackEvent('comparison_view', { from_product: topProduct.name });
            onCompare();
          }}
          className="ml-4 bg-gray-100 border-2 border-gray-300 text-gray-700 font-bold py-3 px-8 rounded-lg hover:bg-gray-200 transition-colors duration-300"
        >
